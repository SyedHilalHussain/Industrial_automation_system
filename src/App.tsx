...
const ensureSingleExitForShop = (stations: StationTopology[], shopId: number): StationTopology[] => {
  ...
  return stations.map((st, idx) => {
    // If successor is already defined and points to a valid station in this shop, 'exit', or an OR Gate, preserve it!
    const isValidSuccessor = st.successor && (
      st.successor === 'exit' ||
      st.successor.startsWith('orgate-') ||
      stations.some(s => s.id === st.successor)
    );
    if (isValidSuccessor) {
      return st;
    }
...
export default function App() {
  const [currentStep, setCurrentStep] = useState<'configuration' | 'layout' | 'shop-layout' | 'simulation'>(() => {
    const saved = localStorage.getItem('industrial_layout_current_step');
    if (saved === 'configuration' || saved === 'layout' || saved === 'shop-layout' || saved === 'simulation') {
      return saved;
    }
  ...
  const defaultShops: ShopTopology[] = [
    ...
    const shapesList: Array<'pentagon' | 'heart' | 'square' | 'triangle' | 'diamond' | 'oval' | 'rectangle'> = [
      'square', 'rectangle', 'triangle', 'oval'
    ];
    const colorsList = ['#FF5733', '#4B8EFF', '#EB9E34', '#3ECB85'];
    return {
      ...shop,
      status: shop.status as 'Active' | 'Standby' | 'Ready' | 'Idle',
      stationsData: createDefaultStationsForShop(shop.id, shop.stations),
      partShape: shapesList[(shop.id - 1) % shapesList.length],
      partSize: 50,
      partWidth: 50,
      partHeight: 50,
      paintStrokeWidth: 3,
      paintFillColor: colorsList[(shop.id - 1) % colorsList.length],
    };
  });

  const [shops, setShops] = useState<ShopTopology[]>(() => {
    const saved = localStorage.getItem('industrial_layout_shops');
    if (saved) {
      try {
        ...
        console.error("Error parsing saved shops:", e);
      }
    }
    return arrangeShopsInSequentialFlow(defaultShops);
  });

  const [savedProjects, setSavedProjects] = useState<any[]>(() => {
    const saved = localStorage.getItem('industrial_saved_projects');
    if (saved) {
      try {
  ...
  const handleSaveProject = (name: string, step?: string) => {
    const newProj = {
    ...
    const updated = [...savedProjects, newProj];
    setSavedProjects(updated);
    localStorage.setItem('industrial_saved_projects', JSON.stringify(updated));
    return newProj;
  };
  ...
  const handleLoadProject = (id: string) => {
    ...
    if (proj) {
      setShops(proj.shops);
      setCurrentStep(proj.currentStep || 'simulation');
      localStorage.setItem('industrial_layout_shops', JSON.stringify(proj.shops));
      localStorage.setItem('industrial_layout_current_step', proj.currentStep || 'simulation');
    }
  };
  ...
  const handleRenameProject = (id: string, newName: string) => {
    const updated = savedProjects.map(p => p.id === id ? { ...p, name: newName || p.name } : p);
    setSavedProjects(updated);
    localStorage.setItem('industrial_saved_projects', JSON.stringify(updated));
  };
  ...
  const handleDeleteAllProjects = () => {
    setSavedProjects([]);
    localStorage.setItem('industrial_saved_projects', JSON.stringify([]));
  };

  // Sync state on change
  React.useEffect(() => {
    localStorage.setItem('industrial_layout_current_step', currentStep);
  }, [currentStep]);

  React.useEffect(() => {
    localStorage.setItem('industrial_layout_shops', JSON.stringify(shops));
  }, [shops]);
  ...
  const handleProceedFromConfiguration = (config: {
    ...
    const generatedShops: ShopTopology[] = Array.from({ length: config.shopCount }).map((_, idx) => {
      ...
      return {
        ...
        partShape: shapesList[(shopId - 1) % shapesList.length],
        partSize: 50,
        partWidth: 50,
        partHeight: 50,
        paintStrokeWidth: 3,
        paintFillColor: colorsList[(shopId - 1) % colorsList.length],
      };
    });
